...
const Camera =  (props) => {
    ...
    useEffect(() => {
        ...
        async function getInfoFromMedia() {
            ...
            m++
            const imageDetection = await faceapi.detectAllFaces(videoRef.current).withFaceLandmarks().withFaceDescriptors().withAgeAndGender()
            ...
            let detectOneFace = await faceapi.detectAllFaces(img).withFaceLandmarks().withFaceDescriptors().withAgeAndGender()

            console.log("detectOneFace", detectOneFace);
            ...
            if (fullFaceDescriptions.length) {
              ...
              console.log("resultsImageDetection", resultsImageDetection);
         
              const context = canvasRef.current.getContext("2d");
              context.clearRect(0, 0, context.canvas.width, context.canvas.height);

              handleCaptureAge(imageDetection,resultsImageDetection) 
    ...
    function handleCaptureAge(imageDetection,resultsImageDetection) {
        const context = canvasRef.current.getContext("2d");
        context.clearRect(0, 0, context.canvas.width, context.canvas.height);
        ...
        imageDetection.forEach((prediction,i) => {

          
            console.log("----- prediction : ",i,  prediction.detection.box.x);
            const x = prediction.detection.box.x;
            const y = prediction.detection.box.y;
            const width = prediction.detection.box.width;
            const height = prediction.detection.box.height;
            ...
            context.fillRect(x, y, textWidth + 10, textHeight);
            // draw bottom left rectangle
            context.fillRect(x, y + height - textHeight, textWidth + 15, textHeight);

            context.fillStyle = "#FFFFFF";
            context.fillRect(x, y + height, textWidth + 15, textHeight);
      
            // Draw the text last to ensure it's on top.
            context.fillStyle = "#000000";
            context.fillText(prediction.age.toFixed(2), x, y);
            context.fillText(prediction.gender, x, y + height - textHeight);
            context.fillText(resultsImageDetection[i]._label, x, y + height);
          });
    }    
